...
function woostifyInfiniteScroll( addEventClick, infScrollPath ) {
	...
	const loading_status = view_more_btn_wrap.querySelector( '.woostify-loading-status' ),
	...
	infScroll.on(
		...
		function( body, path, fetchPromise ) {
			const all_page = body.querySelectorAll( '.woocommerce-pagination .page-numbers .page-numbers:not(.next):not(.prev):not(.dots)' );
			const next_page_el = body.querySelectorAll( '.woocommerce-pagination .page-numbers .page-numbers.next' );
			const is_last_page = ( ! next_page_el.length ) ? true : false;
